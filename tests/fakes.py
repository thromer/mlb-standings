...
class FakeSheet(SheetLike):
    ...
    def get_range(
        self, sheet_range: str, major_dimension: Dimension = "ROWS"
    ) -> SheetArray:
    ...
    def transpose(values: SheetArray) -> SheetArray:
        ...
        for col in values[1:]:
            if len(col) != n_columns:
                msg = f"{values} is not rectangular"
                raise ValueError(msg)
    ...
    def set_range(self, sheet_range: str, values: SheetArray) -> None:
        ...
        if self.r_max < r_max:
            self.values.extend(
                [[""] * (self.c_max + 1) for _ in range(r_max - self.r_max)]
            )
            self.r_max = r_max
        if self.c_max < c_max:
...
@final
class FakeSpreadsheet(SpreadsheetLike):
...
@final
class FakeSpreadsheets(SpreadsheetsLike):
...
class FakeWeb(WebLike):
    ...
    def read(self, url: str) -> str:
        ...
        if (
            ...
            url
            == "https://statsapi.mlb.com/api/v1/schedule/?sportId=1&leagueId=103,104&scheduleTypes=games&gameTypes=R&fields=dates,games,officialDate&startDate=01/01/2023&endDate=12/31/2023"
        ):
            fname = f"{self.data_dir}/schedule-for-first-day.json"
        else:
            m = re.match(
                r"https://www.baseball-reference.com/boxes/\?year=(\d{4})&month=(\d{2})&day=(\d{2})",
                url,
            )
            if m:
        ...
        if fname is None:
            msg = f"{url} unsupported by FakeWeb"
            raise ValueError(msg)
