...
class Updater:
    ...
    def _build_contents(contents_spreadsheet: SpreadsheetLike) -> dict[int, str]:
        return {
            int(row[0]): str(row[1])
            for row in contents_spreadsheet.get_range("contents")
    ...
    def create_spreadsheet_for_year(self, year: int) -> None:
        # TODO: hack: using recursive to avoid trying to create previous year's spreadsheet
        last_year_spreadsheet_id = self.get_spreadsheet_id_for_year_recursive(
            year - 1, 1
        )
        ...
        print(
            f"Created {new_name} https://docs.google.com/spreadsheets/d/{new_id}/edit"
        )
    ...
    def update_regular_season(self) -> SeasonStatus:
        ...
        * Backfill: for date in [spreadsheet_max_date + 1, 'today']
        ...
        """
        first_day_val = self.spreadsheet.get_cell(_FIRST_DAY)
        if type(first_day_val) is int:
            ...
            self.spreadsheet.set_cell(_FIRST_DAY, date_to_excel_date(first_day))
        last_day_val = self.spreadsheet.get_cell(_LAST_DAY)
        last_day = (
            date_from_excel_date(last_day_val) if type(last_day_val) is int else None
        )
        if last_day is not None:
            return SeasonStatus.OVER
        print(f"last_day before {last_day}")
        column_as = {
            league: self.spreadsheet.get_range(
                f"{self._upload_sheet_name(league)}!A:A", major_dimension="COLUMNS"
            )[0]
        ...
        for league in _LEAGUES:
            ...
            if len(column_as[league]) == 0:
                ...
                self.spreadsheet.set_range(
                    f"{self._upload_sheet_name(league)}!{rc0_range_to_sheet_range(((0, 0), (0, len(row))))}",
        ...
        first_day_to_upload = max(
            first_day - _ONE_DAY, min(newest_league_upload_day.values()) + _ONE_DAY
        )
        last_day_to_upload = (
            date.fromordinal(self.now.toordinal()) - _ONE_DAY
        )  # Ugly, assumes we already did the conversion to America/Los_Angeles
        ...
        for day in [
            first_day_to_upload + timedelta(days=d)
            ...
            for league in _LEAGUES:
                ...
                sheet_range = rc0_range_to_sheet_range(
                    ((row_index, 0), (row_index, len(values) - 1))
                )
                self.spreadsheet.set_range(
                    f"{self._upload_sheet_name(league)}!{sheet_range}", [values]
                )
        ...
        if last_day is None and last_day_uploaded != last_day_to_upload:
            last_day = self.baseballref.last_scheduled_regular_day(
                min(newest_league_upload_day.values())
            )
            if last_day < self.now.date():
                print("update _LAST_DAY")
                self.spreadsheet.set_cell(_LAST_DAY, date_to_excel_date(last_day))
                return SeasonStatus.OVER
        ...
        #     spreadsheet.set_cell(_LAST_DAY, date_to_excel_date(last_day))

    def update_post_season(self) -> SeasonStatus:
        # TODO: I don't think this is quite robust but ok.
        last_post_season_day_val = self.spreadsheet.get_cell(_LAST_POST_SEASON_DAY)
        last_post_season_day = (
            date_from_excel_date(last_post_season_day_val)
            if type(last_post_season_day_val) is int
            else None
        )
    ...
    def _get_newest_league_day(
        self, column: list[SheetValue], league: str, first_day: date
    ) -> date:
        ...
        for sheet_val in column[1:]:
            if type(sheet_val) is not int:
                msg = f"Unexpected value {sheet_val} in column A2:A"
                raise ValueError(msg)
            got = date_from_excel_date(sheet_val)
            if type(sheet_val) is not int or got != want:
                msg = f"Expected date {want} in {self._upload_sheet_name(league)} but got {got}"
                raise ValueError(msg)
            want = want + _ONE_DAY
    ...
    def announce_season_opened(self) -> None:
        year = self.now.year
        msg = EmailMessage()
