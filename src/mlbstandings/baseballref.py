...
class Standings:
    def __init__(self, league: str, stats: dict[str, dict[str, float]]):
        ...
        self.div_orders: dict[str, list[str]] = {
            ...
            division: sorted(
                sorted(self.divisions[division]), key=lambda team: -stats[team]["pct"]
            )
            for division in self.divisions.keys()
        }

        wc_teams = set(self.all_teams) - sole_leaders
        self.wc_order: list[str] = sorted(
            sorted(wc_teams), key=lambda team: -stats[team]["pct"]
        )
    ...
    def row(self) -> list[str | int]:
        pm = self.plus_minus()
        div_orders: list[str] = list(
            itertools.chain.from_iterable([self.div_orders[d] for d in DIVISION_ORDER])
        )
...
class BaseballReference:
    ...
    def first_day(self, year: date) -> date:
        ...
        y = year.year
        if year != date(y, 1, 1):
            msg = f"year field should be for January 1, is {year}"
    ...
    def spreadsheet_row(self, day: date) -> dict[str, list[str | int]] | None:
        ...
        url = day.strftime(
            "https://www.baseball-reference.com/boxes/?year=%Y&month=%m&day=%d"
        )
        ...
        if today_button is None or type(today_button) is not bs4.element.Tag:
            msg = f"Unable to find current date in {url}"
            raise ValueError(msg)
        ...
        #    return None
        return {league: self._work(league, soup).row() for league in LEAGUES}
    ...
    def _work(self, league: str, soup: bs4.BeautifulSoup) -> Standings:
        ...
        if overall_table is None or type(overall_table) is not bs4.element.Tag:
            msg = f"{table_id} is missing or exists with no tbody"
            raise ValueError(msg)
    ...
    def zeroday() -> dict[str, list[str | int]] | None:
        stats = {
            abbr: {"w": 0, "l": 0, "pct": 0.5}
            for abbr in itertools.chain.from_iterable(LEAGUE_TEAMS.values())
        }
    ...
    def header_row(league: str) -> list[SheetValue]:
        ...
        return list(
            itertools.chain.from_iterable(
                ...
                + [
                    [f"{league} {x}"] + [""] * 4
                    for x in [_CANONICAL_DIVISION_NAMES[div] for div in divs.keys()]
                ]
                + [["Wildcard"]]
            )
        )
